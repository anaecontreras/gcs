...
class DocumentoController extends Controller
{
    ...
    /**
 * HU-16: Búsqueda y filtrado de documentos
 * Método para filtrar por palabras clave, categoría y fechas
 */
public function filtrar(Request $request)
{
    ...
    // Filtro por palabras clave (búsqueda en título y versión)
    ...
    // Filtro por usuario creador (opcional, útil para administradores)
    if ($request->filled('usuario_id')) {
        $query->where('usuario_creador_id', $request->input('usuario_id'));
    }

    // Ordenar por fecha de publicación descendente (más recientes primero)
    $documentos = $query->orderBy('fecha_publicacion', 'desc')
                        ->paginate(10);

    // Agregar URL completa para cada documento
    $documentos->getCollection()->transform(function ($doc) {
        $doc->url_archivo = asset('storage/' . $doc->ruta_archivo);
        return $doc;
    });

    // Registrar actividad (logs) si hay usuario autenticado
    if ($request->user()) {
        \App\Services\LogActivityService::registrar(
